...
class CsvProfileView(QWidget):
    ...
    def _wire_widget_events(self) -> None:
        ...
        self.ui.button_browse_csv_conversion_path_input.clicked.connect(self._on_browse_input)
        self.ui.button_profile_cleanup_browse_csv_file_path.clicked.connect(self._on_browse_profile_csv)
    ...
    def _on_browse_input(self) -> None:
        ...
        file_path, _ = QFileDialog.getOpenFileName(self.window, "Select File for Target Conversion", "", "All Assets (*.*)")
        if file_path:
            self.ui.line_edit_csv_conversion_path_input.setText(file_path)

    def _on_browse_profile_csv(self) -> None:
        file_path, _ = QFileDialog.getOpenFileName(self.window, "Open Visualization Dataset", "", "CSV Target Logs (*.csv)")
        if file_path:
            ...
            try:
                ...
                QMessageBox.critical(self.window, "I/O Error", f"Failed to mount file structure: {str(ex)}")
