...
class XMLuvationConfig:
    ...
    def load_all(self) -> None:
        ...
        super().__setattr__("csv_conversion_input", str(settings.value("csv_conversion_input", "")))
        super().__setattr__("profile_cleanup_csv_path", str(settings.value("profile_cleanup_csv_path", "")))
        super().__setattr__("current_theme", str(settings.value("current_theme", "dark")))
        super().__setattr__("max_threads", int(settings.value("max_threads", 4)))
        ...
        super().__setattr__("xpath_expressions", list(saved_xpaths) if saved_xpaths else [])
